...
//
// /**
//  * Контроллер общих вспомогательных методов для регистрации.
//  */
...
//   ): Promise<{ registrationTimeOut: number }> {
//     // Проверка почты на корректность и доступность
//     const emailVerdict = await this.emailAvailability(email);
//     assert(emailVerdict.verdict === 'ok', new NotAcceptableException(emailVerdict));
//
//     // Сгенерировать код
//     const code = await this.securityService.generateSingleUseCode();
